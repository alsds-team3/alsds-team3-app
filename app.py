...
# NOTE: The /admin/migrate trigger is intentionally disabled (per professor's
# feedback). A one-click migration endpoint live on a production app is
# unsafe — anyone hitting the URL could re-run the migration. When a fresh
# migration is needed: uncomment the trigger below, deploy, run it, then
# re-comment and redeploy.
#
# /admin/migrate/status is kept ACTIVE on purpose — it is read-only and lets
# the team verify the last migration's state without redeploying.
